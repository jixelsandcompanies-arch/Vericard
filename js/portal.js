const state = { token: '', org: null, rules: null, orgRegistrationFields: {}, cards: [], templates: [], locked: false, palette: { primary: '#061a30', accent: '#149ee8', colors: ['#061a30', '#149ee8'] }, masterToken: new URLSearchParams(location.search).get('master') || '', dashboardDeviceId: localStorage.getItem('vericardDashboardDeviceId') || '' };
    ...
    function renderSchoolBackFields() {
      ...
      els.schoolHourFields.innerHTML = type === 'school'
        ? '<label>School type<select name="schoolType"><option value="day">Day school</option><option value="boarding">Boarding school</option><option value="mixed">Mixed day/boarding school</option></select></label><label>Weekend release allowed<select name="weekendReleaseAllowed"><option value="no">No</option><option value="yes">Yes</option></select></label><label>Official/exeat release periods<textarea name="releasePeriods" placeholder="2026-08-01 to 2026-08-03, or one range per line"></textarea></label><label>Holiday/release notes<textarea name="holidayNotes" placeholder="Holiday/release reason notes"></textarea></label><label>School start time<input name="schoolStartTime" type="time" value="08:00"></label><label>School end time<input name="schoolEndTime" type="time" value="16:00"></label>'
        : '';
    }
